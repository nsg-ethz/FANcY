...
def print_fancy(pkt):
    ...
    if (reverse_actions[fancy.action] == "MULTIPLE_COUNTERS"):
        ...
        print("FANCY HEADER: ID={}, C/A/F={}{}{}, ACTION={}, COUNT={}, NEXT=0x{:04x}".format(fancy.id,
              fancy.count_flag, fancy.ack, fancy.fsm, reverse_actions[fancy.action], fancy.counter_value, fancy.nextHeader))

        payload = bytes(counters_length.payload)
        for i in range(len(payload) / 4):
            if i == counters_length.length:
                break
            print("counter {} {}".format(counters_length.length - i,
                  int(payload[i * 4:((i + 1) * 4)].encode('hex'), 16)))

    elif (reverse_actions[fancy.action] == "COUNTER" and fancy.counter_value >= 0 and fancy.ack == 1):
        print(
            bcolors.WARNING +
            "FANCY HEADER: ID={}, C/A/F={}{}{}, ACTION={}, COUNT={}, NEXT=0x{:04x}".
            format(
                fancy.id, fancy.count_flag, fancy.ack, fancy.fsm,
                reverse_actions[fancy.action],
                fancy.counter_value, fancy.nextHeader) + bcolors.ENDC)
    else:
        print("FANCY HEADER: ID={}, C/A/F={}{}{}, ACTION={}, COUNT={}, NEXT=0x{:04x}".format(fancy.id,
              fancy.count_flag, fancy.ack, fancy.fsm, reverse_actions[fancy.action], fancy.counter_value, fancy.nextHeader))
