...
def create_tests(fixed_parameters, variable_parameters, out_dir):
    ...
    for fixed_parameter in fixed_parameters:
        ...
        for variable_parameter in dict_product(variable_parameters):
            ...
            # computes rate and flows per sec in a different way
...
def generate_ns3_runs(
    ...
    # build commands
    for run in runs:
        cmd = './waf --run  "main --DebugFlag=false --PcapEnabled=False --FailTime={} --TrafficStart={} --InDirBase={} --EnableSaveDrops=false --SoftDetectionEnabled=false --CheckPortStateEnable=false --TrafficType=StatefulSyntheticTraffic --SwitchType=Fancy --EnableNat=true --NumReceivers=25 --NumSendersPerRtt=25 --PacketHashType=DstPrefixHash --NumDrops={}'.format(
